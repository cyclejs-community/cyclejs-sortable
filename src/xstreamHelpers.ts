 ...
 */
export function emitBetween<T>(
    start$: Stream<any>,
    end$: Stream<any>
): Transform<T, T> {
    //TODO: implement a xstream operator and make a PR
    return s =>
        xs
            .combine(s, start$.startWith(undefined), end$.startWith(undefined))
            .fold(
                (acc, curr) => {
                    const startChanged: boolean = curr[1] !== acc[2];
                    const endChanged: boolean = curr[2] !== acc[3];
                    const shouldEmit: boolean = startChanged
                        ? true
